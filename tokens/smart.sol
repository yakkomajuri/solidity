...
contract Storage {
    ...
    mapping(bytes32 => uint8) public allMyUsers;
...
contract SmartLend is Storage {
    ...
    function payBackLoan(
        ...
        if (alreadyExists == false) {
            ...
            users[_lendee].nUsers = allMyUsers[_lendee] - 1;
            users[_lender].nUsers = allMyUsers[_lender] - 1;
        }
    ...
    function score(
        ...
        view
        returns (uint) {
            uint loanPctg = uint(users[_user].totalPaid / users[_user].loansTotal);
            uint nUsersPctg = users[_user].nUsers / maxN;
            uint nLoansPctg = users[_user].nLoans / maxLoans;
            uint timePctg = (block.timestamp - users[_user].timestampJoined) / maxTime;
